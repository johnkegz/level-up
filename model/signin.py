...
class Signin(object):
    ...
    def submit_inputs(self):
        ...
        if (feilds == True) and (valid_email ==True) and (valid_phone == True) and (valid_password == True):
            ...
            return "YOU HAVE LOGGED IN SUCCESS FULLY"
        else:
            error_list = [feilds, valid_phone, valid_email, valid_password]
            error = [x for x in error_list if x != True]
            print(error)
sign = Signin(f_name = "kalyango", l_name = "john", phone_number = '0752067415', email = 'johnkal24@gmail.com', password = 'kalyango1', submit = 'submit')
sign.submit_inputs()
